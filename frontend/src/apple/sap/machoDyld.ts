...
export class BrowserMachODyldImage {
  ...
  private applyRebases(loadBase: number): number {
    ...
    const emit = () => {
      ...
      const current = readUint64Big(this.data, fileOffset);
      const base = BigInt(this.baseAddress);
      if (current < base) {
        throw new Error(
          `rebase at ${hex(segmentOffset)} contains pointer 0x${current.toString(16)} below image base`,
        );
      }
      const relocated = BigInt(loadBase) + current - base;
      writeUint64Big(this.data, fileOffset, relocated, `${this.name} rebase pointer`);
      count++;
      ...
      try {
        switch (opcode) {
          ...
          case REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB: {
            ...
            const value = readUlebNumber(stream, cursor, "rebase segment offset");
            cursor = value.next;
            segmentOffset = value.value;
            break;
          }
          case REBASE_OPCODE_ADD_ADDR_ULEB: {
            const value = readUlebNumber(stream, cursor, "rebase address delta");
            cursor = value.next;
          ...
          case REBASE_OPCODE_DO_REBASE_ULEB_TIMES: {
            const iterations = readUlebNumber(stream, cursor, "rebase count");
            cursor = iterations.next;
            assertFixupCount(iterations.value, "rebase count");
          ...
          case REBASE_OPCODE_DO_REBASE_ADD_ADDR_ULEB: {
            emit();
            const skip = readUlebNumber(stream, cursor, "rebase skip");
            cursor = skip.next;
            segmentOffset = checkedAdd(
              segmentOffset,
  ...
  private applyBindStream(
    ...
    const resetLazyBinding = () => {
      ...
      symbol = "";
      addend = 0n;
      // Match dyld/go-macho: segOffset is state outside the lazy bind record.
    };
    const emit = () => {
      ...
      try {
        switch (opcode) {
          ...
          case BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB: {
            segmentIndex = immediate;
            const value = readUlebNumber(stream, cursor, "bind segment offset");
            cursor = value.next;
            segmentOffset = value.value;
            break;
          }
          case BIND_OPCODE_ADD_ADDR_ULEB: {
            const value = readUlebNumber(stream, cursor, "bind address delta");
            cursor = value.next;
          ...
          case BIND_OPCODE_DO_BIND_ADD_ADDR_ULEB: {
            emit();
            const skip = readUlebNumber(stream, cursor, "bind skip");
            cursor = skip.next;
            segmentOffset = checkedAdd(
              segmentOffset,
              POINTER_SIZE + skip.value,
          ...
          case BIND_OPCODE_DO_BIND_ADD_ADDR_IMM_SCALED:
            ...
            segmentOffset = checkedAdd(
              segmentOffset,
              POINTER_SIZE + immediate * POINTER_SIZE,
              `${label} offset`,
            );
            break;
          case BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB: {
            const iterations = readUlebNumber(stream, cursor, "bind count");
            cursor = iterations.next;
            const skip = readUlebNumber(stream, cursor, "bind skip");
            cursor = skip.next;
            assertFixupCount(iterations.value, "bind count");
            for (let index = 0; index < iterations.value; index++) {
              emit();
              segmentOffset = checkedAdd(
                segmentOffset,
  ...
  private segmentFileOffset(segmentIndex: number, offset: number, size: number): number {
    ...
    if (end > segment.size) {
      throw new Error(`fixup at ${hex(offset)} exceeds segment ${segment.name}`);
    }
    if (end > segment.fileSize) {
      throw new Error(`fixup at ${hex(offset)} exceeds file data for segment ${segment.name}`);
    }
...
function sliceRange(
  ...
  if (!Number.isSafeInteger(offset + size) || offset < 0 || size < 0 || offset + size > data.length) {
    throw new Error(`${label} stream exceeds image`);
  }
  return data.subarray(offset, offset + size);
}

function readUlebNumber(
  input: Uint8Array,
  start: number,
  label: string,
  ...
  if (result.value > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error(
      `${label} ULEB128 at +${hex(start)} exceeds JavaScript safe integer range (0x${result.value.toString(16)})`,
    );
  }
...
function readUlebBig(
  ...
  for (let index = 0; index < 10; index++) {
    if (cursor >= input.length) throw new Error(`truncated ${label} ULEB128 at +${hex(start)}`);
...
function readSlebBig(
  ...
  for (let index = 0; index < 10; index++) {
    if (cursor >= input.length) throw new Error(`truncated ${label} SLEB128 at +${hex(start)}`);
    byte = input[cursor++];
    value |= BigInt(byte & 0x7f) << shift;
    shift += 7n;
    if ((byte & 0x80) === 0) {
...
function readCStringWithNext(
  ...
  if (end >= input.length) throw new Error(`unterminated bind symbol at +${hex(start)}`);
  return {
...
function writeUint64Big(
  ...
  if (value < 0n || value > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error(`${label} result 0x${value.toString(16)} exceeds guest safe address range`);
  }
  if (offset < 0 || offset + 8 > input.length) {
...
function assertFixupCount(value: number, label: string) {
  if (!Number.isSafeInteger(value) || value < 0 || value > MAX_FIXUPS) {
    throw new Error(`${label} ${value} exceeds safety limit`);
  }
}
